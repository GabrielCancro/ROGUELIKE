...
onready var DunRooms=get_node("/root/Node2D/DungeonRooms")
...
onready var fog_map=get_node("/root/Node2D/Nav2D/FogMap")

#indices de los tiles, por si los necesitas cambiar o algo
#							0							11							12
var tiles={ "LIBRE":5, "MURO":0, "SUELO":1, "SUELO2":6,  "BLOQUEADO":0, "PASILLO":1, "INICIO":2, "SALIDA":3 , "PUERTA":4 }

var traversablesIndexs=[1,2,3,6,92]
...
func generateDungeon(MAP="PROCEDURAL"):
	Globals.TilemapManager.remove_all_elements()
	ROOMS=[]
	if MAP=="PROCEDURAL": generateProceduralDungeon()
	elif MAP=="TUTORIAL": DunRooms.generate_tutorial_dungeon()
	draw_map(DATAMAP)
	tile_map.update_bitmask_region(Vector2(1,1),Vector2(map_w,map_h))
	iniFog()

func generateProceduralDungeon():
	map_w=25+NIVEL*3
	map_h=25+NIVEL*2
	cant_rooms=floor(map_w*map_h/170)+1
	...
	for i in range(cant_rooms):
		create_room( DunRooms.get_room_empty_rnd() )
		if showProcess: draw_map(DATAMAP)
	...
	for RM in ROOMS: DunRooms.create_room_objects(RM)
	addAltar()
	addFuente()
	addEnemies()
	addChests()
	
	POS_INI=Vector2(ROOMS[0]["cx"],ROOMS[0]["cy"])
	return true
...
func addRoomToMap(r):
	#r={x y cx cy w h data=[]}
	for x in range(0,r["w"]):
		for y in range(0,r["h"]):
			DATAMAP[r["x"]+x][r["y"]+y]=r["data"][x][y]
	ROOMS.append(r)
...
func iniFog():
	for x in range(map_w):
		for y in range(map_h):
			get_node("/root/Node2D/Nav2D/FogMap").set_cell(x,y,tiles["MURO"])
...
func isViableDoor(xx,yy):
	if DATAMAP[xx][yy]!=tiles["LIBRE"] and DATAMAP[xx][yy]!=tiles["PASILLO"] : 
		return false
