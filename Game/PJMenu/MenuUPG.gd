...
func drawInfoItems():
	for i in range(0, buttons.size()):
		if i+base<packs.size():
			if packs[i+base]["type"]=="ATTR": buttons[i].get_node("icon").set_frame(90)
			elif packs[i+base]["type"]=="HAB": 
				var myHab=Globals.HabsManager.getHab( packs[i+base]["elem"] )
				if  myHab["type"]=="PAS": buttons[i].get_node("icon").set_frame(92)
				elif  myHab["type"]=="ACT": buttons[i].get_node("icon").set_frame(93)
			elif packs[i+base]["type"]=="SPL": buttons[i].get_node("icon").set_frame(91)
...
func show_current_habs():
	...
	if index+base<packs.size():
		if packs[index+base]["type"]=="SPL" or packs[index+base]["type"]=="HAB": 
			...
			TXT+=realHab["name"]+": "+Globals.HabsManager.getDesc(realHab["code"],packs[index+base]["next_cnt"])
	$habs.set_text(TXT)
