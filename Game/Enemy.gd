...
func dead():
	...
	var sc=ATTRIBUTABLE.get_attr("hp")
	sc+=ATTRIBUTABLE.get_attr("pp")*3
	sc+=ATTRIBUTABLE.get_attr("atk")*3
	sc+=ATTRIBUTABLE.get_attr("def")*3
	sc+=ATTRIBUTABLE.get_attr("pow")*3
	sc+=ATTRIBUTABLE.get_attr("stp")
	Globals.add_score(sc)
	queue_free()
